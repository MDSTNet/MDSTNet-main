...
class DataEmbedding_st(nn.Module):
    def __init__(self, c_in,enc_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.value_embedding = nn.Sequential(nn.Linear(c_in, d_model),
                                             nn.GELU(),
                                             nn.Dropout(p=dropout),
                                             nn.Linear(d_model, d_model, bias=True),
                                       )
        self.fore_embedding = nn.Sequential(nn.Linear(48, d_model),
                                             nn.GELU(),
                                             nn.Dropout(p=dropout),
                                             nn.Linear(d_model, d_model, bias=True),
                                       )
    ...
    def forward(self, x, fore_x=None, coordinate=None):
        ...
        if coordinate is None:
            ...
            if fore_x is not None:
                ...
                return self.dropout(x), self.dropout(fore_x), self.spatial_embedding()
            else:
                return self.dropout(x),  fore_x, self.spatial_embedding()
            # x = self.value_embedding(x) + torch.unsqueeze(torch.unsqueeze(self.spatial_embedding(coordinate) , dim=0), dim=2) + self.channel_embedding()
        # x: [Batch Variate d_model]
        # return self.dropout(x), self.dropout(fore_x), self.spatial_embedding()
